...
resource "aws_security_group" "alb" {
  ...
  ingress {
    description = "Gateway listener, from the operator's network only"
  ...
  ingress {
    description = "Console listener, from the operator's network only"
...
resource "aws_vpc_security_group_ingress_rule" "service_from_alb_gateway" {
  ...
  description                  = "The gateway's container port, from the load balancer"
  from_port                    = 8000
  to_port                      = 8000
  ip_protocol                  = "tcp"
...
resource "aws_vpc_security_group_ingress_rule" "service_from_alb_ui" {
  security_group_id            = aws_security_group.service.id
  description                  = "The console's container port, from the load balancer"
  from_port                    = 3000
  to_port                      = 3000
  ip_protocol                  = "tcp"
  referenced_security_group_id = aws_security_group.alb.id
